...
| **4a** 🚧 | LiteRT-LM tools: time, calendar, location, `webSearch`/`fetchUrl` (Tavily → Bing/DDG/Google HTML fallback) | unchanged thin client |
...
- Guaranteed web search (Tavily keyless/keyed + HTML SERP fallback; rate limits and bot walls can still fail)
